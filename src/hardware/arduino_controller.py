...
class ArduinoController(ResourceMixin):
    ...
    def start_reading(self, callback: Optional[Callable[[SensorReading], None]] = None):
        ...
        response = self.serial.query("START", response_timeout=2.0)
        if response and "STARTED" in response:
            self.logger.info("Started continuous sensor reading")
        else:
            self.logger.warning(f"Unexpected start response: {response}")
