...
class WeightWorker(QThread, ThreadCleanupMixin):
    ...
    def __init__(self, test_instance):
        ...
        self.register_qthread(self, "weight_worker")

    def run(self):
        """Run the test in background thread"""
        try:
            self.logger.info(f"Starting Weight worker for {self.test_instance.sku}")
            ...
            self.logger.info("Weight worker completed successfully")

        except Exception as e:
            self.logger.error(f"Weight test execution error: {e}", exc_info=True)
